...
//    return future.whenComplete((value, throwable) -> scopedSpan.finish());
  ...
  public static ByteString exportCurrentSpan() {
    return ByteString
        .copyFromUtf8(exportSpan(Tracing.currentTracer().currentSpan()));
  }
   ...
   * @return encoded tracing context.
   */
  public static String exportSpan(brave.Span span) {
  ...
  public static void importSpan(ByteString trace) {
    ...
    if (encodedParent.length() > 0) {
      ...
      Tracing.currentTracer().joinSpan(extract.context()).finish();
    }
  }
  ...
  public static void executeWithTrace(String name, ExecutionWithIOException task) {
    Tracer tracer = Tracing.currentTracer();
    ScopedSpan scopedSpan = tracer.startScopedSpan(name);
    try {
      task.run();
